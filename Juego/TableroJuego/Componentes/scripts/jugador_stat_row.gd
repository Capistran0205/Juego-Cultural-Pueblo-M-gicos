...
func _actualizar_casilla(player_data: Dictionary) -> void:
	# Preferimos el nombre real del estado (lo calcula GameManager con
	# NOMBRES_ESTADOS); si no viene, usamos el texto genérico como respaldo.
	if player_data.has("estado_nombre"):
		lbl_casilla.text = player_data["estado_nombre"]
		return
